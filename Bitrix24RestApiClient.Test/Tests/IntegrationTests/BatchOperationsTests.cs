...
public class BatchOperationsTests : AbstractTest
{
    ...
    {
        var items = Bitrix24.Crm.SmartProcesses.ByEntityId(EntityTypeIdEnum.Deal.EntityTypeId).BatchOperations.ListGetStrategy.ListAll<TestDeal>(x=>x.Id, x => x
            .AddFilter(y => y.Id, 1, FilterOperator.GreateThan)
            .AddFilter(y => y.Id, 100, FilterOperator.LessThan));

        var deals = new List<TestDeal>();
    ...
    {
        var items = Bitrix24.Crm.Deals.BatchOperations.ListGetStrategy.ListAll<Deal>(x=>x.Id, x=>x
    ...
    {
        var items = Bitrix24.Crm.StageHistories.Deals.BatchOperations.ListStrategy.ListItemsAll<DealStageHistory>(x=>x.Id, x => x
            .SetEntityTypeId(EntityTypeIdEnum.Deal)
            .AddFilter(y => y.Id, 132700, FilterOperator.GreateThan));

        var dealStageHistory = new List<DealStageHistory>();
        await foreach (var item in items)
    ...
    {
        var items = Bitrix24.Crm.Products.BatchOperations.ListStrategy.ListAll<Product>(x => x.Id, x => x
