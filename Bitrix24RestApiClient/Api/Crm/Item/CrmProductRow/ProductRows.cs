...
public class ProductRows
{
    ...
    public async Task<UpdateProductRowResponse> Update(int id, Action<IUpdateRequestBuilder<ProductRowNew, ItemUpdateArgs>> builderFunc)
    {
        var builder = new UpdateRequestBuilder<ProductRowNew, ItemUpdateArgs>();
        builder.SetId(id);
        builderFunc(builder);
        return await client.SendPostRequest<object, UpdateProductRowResponse>(entityTypePrefix, EntityMethod.Update, builder.BuildArgs());
    }
